...
function doGet() {
  ...
  const valuesData = sheet.getDataRange().getDisplayValues();
  const headers = valuesData[0]; // Assuming the first row contains headers

  var mappedData = [];
  for (var i = 1; i < valuesData.length; i++) {
    var row = valuesData[i];
    var rowData = {};
  ...
  return ContentService.createTextOutput(JSON.stringify(mappedData)).setMimeType(ContentService.MimeType.JSON);
}
